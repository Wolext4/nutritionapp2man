        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription className="text-sm sm:text-base">Track your meals throughout the day</CardDescription>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => navigateDate("prev")} className="h-8 w-8 p-0">
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <div className="text-sm font-medium min-w-[120px] text-center">{formatDate(selectedDate)}</div>
              <Button
                variant="outline"
                size="sm"
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <CardTitle className="text-sm sm:text-base">{mealType.label}</CardTitle>
                ...
                {mealsForType.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-sm font-medium text-green-600">{totalCalories.toFixed(0)} cal</div>
                    </div>
                    <div className="space-y-1">
                      {mealsForType.slice(0, 3).map((meal) => (
                        <div key={meal.id} className="text-xs text-muted-foreground">
                          <div className="flex justify-between">
                            <span className="truncate">{meal.foods.map((f) => f.name).join(", ")}</span>
                            <span>{meal.totalNutrition.calories.toFixed(0)} cal</span>
                          </div>
                        </div>
                      ))}
              ...
              {meals.map((meal) => (
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-sm font-medium">{meal.foods.map((f) => f.name).join(", ")}</div>
                    <div className="text-xs text-muted-foreground">
                      {meal.foods.length} item{meal.foods.length !== 1 ? "s" : ""}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-medium">{meal.totalNutrition.calories.toFixed(0)} cal</div>
                    <div className="text-xs text-muted-foreground">
